...
- [ ] T001 [P] Criar/confirmar índice GiST em `estacoes_inmet.posicao` (pode já existir da feature 002; validar aqui)
- [ ] T002 [P] Criar/confirmar índice GiST em `talhoes.geometria` (pode já existir da feature 005; validar aqui)
...
- [ ] T003 [US1] Implementar `app/db/queries/estacao_proxima.py`: query usando `ORDER BY posicao <-> ST_Centroid(geometria) LIMIT 1`, com `ST_Distance(..::geography)` para distância real em metros
- [ ] T004 [US1] Implementar endpoint `GET /api/v1/talhoes/{id}/estacao-mais-proxima` (contrato em `contracts/estacao-mais-proxima.md`), protegido por autenticação
- [ ] T005 [US1] Tratar talhão inexistente ou sem estações cadastradas com HTTP 404
...
- [ ] T006 [P] Escrever teste de contrato com dataset de distâncias conhecidas em `tests/contract/test_estacao_mais_proxima.py`
- [ ] T007 Medir e validar tempo de resposta < 100ms (RNF003) com `EXPLAIN ANALYZE` sobre a query do T003
- [ ] T008 Rodar os 2 cenários de `quickstart.md`
